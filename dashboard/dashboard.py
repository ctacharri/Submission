...
def create_sum_rent_items_df(df): #bertangnggung jawab menyiapkan sum_rent_items_df
    sum_rent_items_df = df.groupby("instant").cnt.sum().sort_values(ascending=False).reset_index()
    return sum_rent_items_df

def create_byseason_df(df): #menyiapkan byseaoson_df
...
def create_byworkingday_df(df): #menyiapkan byworkingday_df
...
day_df = pd.read_csv("https://raw.githubusercontent.com/ctacharri/proyek-akhir-analisis-data/main/dashboard/all_data.csv")
...
with st.sidebar:
    ...
    st.image("https://raw.githubusercontent.com/ctacharri/logo/main/logo.jpg")
